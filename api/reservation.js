...
async function persistReservation(b){
  ...
  const r=await fetch(`${SUPABASE_URL}/rest/v1/near_drive_reservations`,{method:'POST',headers:{apikey:SUPABASE_PUBLISHABLE_KEY,Authorization:`Bearer ${SUPABASE_PUBLISHABLE_KEY}`,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify(row)});
  if(!r.ok){const data=await r.json().catch(()=>null);throw new Error(data?.message||`Reservation persistence failed (${r.status})`)}
  return {stored:true};
}
export default async function handler(req,res){
  ...
  try{await persistReservation(b)}catch(e){console.error('NEAR reservation persistence failed',e);return res.status(500).json({ok:false,error:'Reservation could not be stored. Please try again.'})}
  ...
  if(!adminEmail||!resendKey){
    ...
    return res.status(202).json({ok:true,stored:true,emailSent:false,status:'SUPPLIER_PENDING',message:'Reservation stored; admin email delivery is pending email-service configuration.'});
  }
  try{
    ...
    return res.status(200).json({ok:true,stored:true,emailSent:true,status:'SUPPLIER_PENDING',emailId:data.id||null});
  }catch(e){
    console.error('NEAR reservation email failed',e);
    return res.status(202).json({ok:true,stored:true,emailSent:false,status:'SUPPLIER_PENDING',message:'Reservation stored; admin email delivery failed and requires review.'});
  }
}
